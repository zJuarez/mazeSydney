...
void Algorithm::getline(char *buffer, int max_len)
{
  uint8_t idx = 0;
  char c;
  do
...
return false;
    ...
    if(sensors.temperatureCelcius(sensors.mlxLeft) > (sensors.firstTemperature + 10) && I<16)
      {
      ...
      if(sensors.temperatureCelcius(sensors.mlxRight) > (sensors.firstTemperature + 10) && D<16)
      {
        sensors.motors.detenerse();
        sensors.blinkLeds();
...
bool Algorithm::visualVictim(int dI, int dD)
{
  if(dI==357)
  dI=2;
  if(dD==357)
...
int b = 0;
 int D = sensors.getDistanceOf(2);
  int I = sensors.getDistanceOf(1);
  int cont = 0;
  unsigned long tiem = millis();
  bool hV =false;
  bool vV =false;
  bool trueRamp = false;
  
while(sensors.motors.rightCount <1200)
{
if(cont%17==0)
{
D=sensors.getDistanceOf(2);
I=sensors.getDistanceOf(1);
  ...
  sensors.motors.avanzar(sensors.motors.de,I,D,b);
...
unsigned long mm = millis();
      ...
      while(sensors.motors.condRampa(s))
      {
       sensors.motors.avanzar(sensors.motors.de, sensors.getDistanceOf(1), sensors.getDistanceOf(2), b);
...
getline(buffer, max_len);
  ...
  if(buffer[0] == '1' && D<21 && !vV)
  {
     sensors.motors.detenerse();
     sensors.motors.escribirLCD("builto", "bulto");
     delay(2000);
     done = true;
     getline(buffer, max_len);
  }
    if(buffer[0] == '4' && D < 21 && done){
    sensors.motors.escribirLCD("LETRA S", "DERECHA");
    sensors.blinkLeds();;
    return;}
    else if(buffer[0] == '3' && D < 21  && done){
    sensors.motors.escribirLCD("LETRA U", "DERECHA");
    sensors.blinkLeds();
    return;}
    else if(buffer[0] == '2' && D< 21  && done){
    sensors.motors.escribirLCD("LETRA H", "DERECHA");
    sensors.blinkLeds();
    return;}
