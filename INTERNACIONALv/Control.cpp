...
{
  if(temperatureCelcius(mlxLeft) >= 24.8)
  {
    detenerse();
    ...
    while(desalineao(a,newD))
    {
      if(millis()<(t1+3000))
     giroDer(newD);
     else if(millis()<(t1+6000))
     giroD(240);
     else
     break;
     a = getAnguloActual();
  ...
  else if (temperatureCelcius(mlxRight) >= 24.8)
  {
    detenerse();
    banderaTotalCalor=true;
    double newD;
...
if(!banderaTotalVision && !alg)
{
 if(visualVictim(10,10))
 {nepe=millis();
  banderaTotalVision=true;
 }
}
angulo=getAnguloActual();
...
void Control::atrasSN(bool alg)
{
  girosX=0;
  this -> setBase(velInicial-20);
...
void Control::desacomodo2(double deseado, int tiempo)
{
  double a = getAnguloActual();
  unsigned long temp = millis();
  while(desalineao(a,deseado))
  {
    giroD(255);
    a=getAnguloActual();
    if(millis()<(temp+tiempo))
    {
      while(desalineao(a,deseado))
      {
        giroI(255);
        a=getAnguloActual();
        if(millis()<(temp+tiempo+tiempo))
        {
          detenerse();
          delay(400);
          return;
...
bool Control::visualVictim(int dI, int dD)
{
  //return false;
  ...
  //     double newD;
    ...
    /* double a = getAnguloActual();
     double aa=a;
       ...
       int aux = rightCount;
       */
        this->escribirLCD("LETRA S", "DERECHA");
        this->blinkLeds();
       /* unsigned long t3 = millis();
        ...
        while(desalineao(a,de))
        {
          if(millis()<(t4+3000))
          giroDer(de);
          else if(millis()<(t4+6000))
          giroD(240);
          else
          break;
...
*/
    ...
    else if(buffer[0] == 'U' &&dD<18){
       this->detenerse();
    this->escribirLCD("LETRA U", "DERECHA");
        this->blinkLeds();
...
while(mySerial.available()>0){
    ...
    /* double a = getAnguloActual();
     double aa=a;
       ...
       */
        this->escribirLCD("LETRA H", "DERECHA");
        this->blinkLeds();
...
while(mySerial.available()>0){
    ...
    else if(buffer[0]=='u' && dI<18)
    {
             this->detenerse();
    this->escribirLCD("LETRA U", "left");
        this->blinkLeds();
             buffer[0]='0';
while(mySerial.available()>0){
          char t=mySerial.read();}
...
void Control::getline(char *buffer, int max_len)
{
 uint8_t idx = 0;
  char c;
  do
...
void Control::cleanBuffer()
{
  while(mySerial.available()>0){
          char t=mySerial.read();}
}

bool Control::bumperCabron()
{
  double valor = y();
 if((valor>-357 && valor<-330) || (valor<-5 && valor>-12) )
 return true;
 else
return false; 
}
bool Control::cuadroVerde()
{
  tcaselect(1);
  int r, g, b;
...
if(r<400  && b<400 && r>100 && g>100 && b>100)
return true;
else
return false;
}
