...
int inByte =  cam.read();
  ...
  if(inByte == 6){ // 3 ES PARA LAS VICTIMAS H DEL LADO IZQUIERDO
   ...
   else if(inByte == 2 || inByte ==1){ // 5 ES PARA LAS VICTIMAS S DE LADO DERECHO
...
void Algorithm::rightTurn()
{
    ...
    if(sensors.getDistanceOf(sensors.echo_A, 4) <= 21 && !sensors.motors.bumperControl && ((sensors.motors.girosX>2 || sensors.getDistanceOf(sensors.echo_E,3)==0)))
    {
